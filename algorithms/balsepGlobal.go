...
// This requires all subedges to be added explicitly to the input graph.
...
func (b BalSepGlobal) findGHD() Decomp {
	return b.findDecomp(b.Graph)
}

// FindDecomp finds a decomp
func (b BalSepGlobal) FindDecomp() Decomp {
	return b.findDecomp(b.Graph)
}

// FindDecompGraph finds a decomp, for an explicit graph
func (b BalSepGlobal) FindDecompGraph(G Graph) Decomp {
...
func (b BalSepGlobal) findDecomp(H Graph) Decomp {
	...
	edges := FilterVerticesStrict(b.Graph.Edges, append(H.Vertices()))
	generators := SplitCombin(edges.Len(), b.K, runtime.GOMAXPROCS(-1), false)
